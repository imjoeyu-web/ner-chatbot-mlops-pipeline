...
MODEL_FILE = 'models/ner/1/model.savedmodel'
...
history = model.fit(X_train, np.array(y_train), batch_size=32,
                    epochs=5, validation_split=0.1, verbose=1, callbacks=callbacks)
...
with tarfile.open(TAR_FILE, "w:gz") as mytar:
  mytar.add("/models")

with file_io.FileIO(TAR_FILE, mode='rb') as input_f:
  with file_io.FileIO(args.output_model_path + '/' + TAR_FILE, mode='wb+') as output_f:
