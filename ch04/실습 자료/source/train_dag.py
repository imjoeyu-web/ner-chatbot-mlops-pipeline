...
with DAG(
    ...
    containerize = BashOperator(
        ...
        bash_command='cd /opt/airflow/dags/ && bentoml containerize fraud_detection_service:latest -t fraud_detection',
    )

    register_container = BashOperator(
        task_id='register_container',
        bash_command='cd /opt/airflow/dags/ && docker image tag fraud_detection:latest localhost:6000/model_serving:latest && docker push localhost:6000/model_serving:latest',
    )
